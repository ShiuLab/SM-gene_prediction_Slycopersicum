...
class aln_util:
	...
	def do_yn00(self,paml_dir,llist):
		...
		# Check control file
		flist = os.listdir("./")
		if "yn00.ctl" not in flist:
			print "No control file, QUIT!"
			sys.exit(0)
		
		# run paml
		os.system("%s/yn00 >> run.log" % paml_dir)
	...
	def do_clustal(self,id1,id2,fdict,outheader,clus_dir):
		...
		os.system("%s/clustalw %s.FA /OUTFILE=%s.GDE /OUTPUT=GDE >> run.log" % \
				   (clus_dir,outheader,outheader))
		#sys.exit(0)
		return error
	...
	#
	# Break the cluster file into.
	#
	def organize(self,groupfile,by):
		
		print "Build group dict..."
		inp = open(groupfile)
		inl = inp.readlines()
		G   = {} # {group_id:[member]}
		for i in inl:
		...
		print "Build size dict..."
		S   = {} # {group_size:[group_id]}
		for i in G:
			s = len(G[i])
			if s not in S:
				S[s] = [i]
		...
		print "Pick groups..."
		step = len(gorder)/by
		remainder = len(gorder) % by
		g = []
		for i in range(0,by):
			#print i
			t = []
			for j in range(0,step):
				#print "",i+j*by, gorder[i+j*by]
				t.append(gorder[i+j*by])
			#print g
			g.append(t)
		
		if remainder > 0:
			g[-1].extend(gorder[-(remainder):])
			#print ">>",g
		
		print "Output..."
		c = 1
		for i in g:            # each meta-group
			os.system("mkdir group%i" % c)
			oup = open("group%i/group%i" % (c,c),"w")
			for j in i:        # each group
				for k in G[j]: # each seqid
					oup.write("%s\t%s\n" % (j,k))
			c += 1
	...
	def help(self):
		...
		print "        pep,cds, OPTIONAL: p,paml,clutsal,window,step,gdom,"
		print "        pairs,ks,justka,x4,debug"
		print "     rate_pair2 - much simplified. Assume cds passed have been"
		print "        aligned. Process the output of Translation.tl_mindless()"
		print "        NEED: pairs,cds, OPT: prog, paml"
		...
		print "     organize - sort based on the group size then generate new"
		print "        group files for batch operation. NEED: group, by"
		print " -p     program to use, paml or tpl"
		print " -pep   protein sequence file"
		print " -cds   coding seq file"
		...
		print " -by    break the group file into this number of subfiles"
		print ""
		sys.exit(0)
...
if __name__ == '__main__':
	...
	function = pep = cds = pairs = group = gdom = clade = pick = aln_profile = ""
	keep = window  = justka = bootstrap = isdir = x4 = debug = 0
	step = mode = sp = 1
	clustal ="/bin"
	prog    ="paml"
	paml    ="/home/shiu/bin/paml4/bin/"
	wdir    ="./"
	ks      ="full"
	phydir = ""
	alndir = ""
	...
	if function == "rate_pair":
		if pep == "" or cds == "":
			print "\nNeed pep and cds fasta file, dirs\n"
			util.help()
		util.rate_pair(pep,cds,pairs,clustal,prog,paml,window,step,gdom,ks,
						justka,x4,debug)
						
	elif function == "rate_pair2":
		if pairs == "" or cds == "":
			print "\nNeed pair and cds file\n"
			util.help()
		util.rate_pair2(pairs,cds,prog,paml)
		...
		if pep == "" or cds == "":
			print "\nNeed pep and cds fasta file\n"
			util.help()
		util.bt_align(pep,cds)
	...
	elif function == "organize":
		if "" in [group,by]:
			print "\nNeed group file and divided by info\n"
			util.help()
		util.organize(group,by)
